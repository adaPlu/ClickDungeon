...
public class SaveManager {
    ...
    private CharacterProfile copyProfile(CharacterProfile profile) {
        ...
        CharacterProfile copy = new CharacterProfile(profile.getName(), profile.getPlayerClass());
        copy.setStrength(profile.getStrength());
        copy.setIntelligence(profile.getIntelligence());
        copy.setConstitution(profile.getConstitution());
        copy.setDexterity(profile.getDexterity());
        copy.setLevel(profile.getLevel());
        copy.setXp(profile.getXp());
        copy.setAvailableStatPoints(profile.getAvailableStatPoints());
        String weapon = profile.getEquippedWeaponName();
        if (weapon != null) {
            copy.equipWeapon(weapon);
        }
        String armor = profile.getEquippedArmorName();
        if (armor != null) {
            copy.equipArmor(armor);
        }
        copy.setCurrentHP(profile.getCurrentHP());
        copy.setCurrentMP(profile.getCurrentMP());
        return copy;
    }
