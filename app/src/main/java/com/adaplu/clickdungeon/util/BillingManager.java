...
public class BillingManager implements PurchasesUpdatedListener {
    ...
    private void handlePurchases(List<Purchase> purchases) {
        ...
        for (Purchase p : purchases) {
            ...
            // Avoid logging raw product identifiers to reduce information exposure.
            try {
                int productCount = p.getProducts() == null ? 0 : p.getProducts().size();
                Log.i(TAG, "Found purchase (product_count): " + productCount);
            } catch (Exception ex) {
                Log.i(TAG, "Found purchase (product_count): unknown");
            }
            if (!p.isAcknowledged()) {
