...
public class InventoryManager {
    ...
    public static synchronized void saveInventory(Context context, List<InventoryItem> inventory) {
        ...
        boolean ok = PersistedBlobStore.save(context, PREFS_NAME, INVENTORY_KEY, INVENTORY_SCHEMA_VERSION, json);
        if (!ok) {
            android.util.Log.w("InventoryManager", "Failed to persist inventory");
        }
    }
    ...
    public static synchronized java.util.List<InventoryItem> loadPremiumInventory(Context context) {
        ...
        return json != null ? GSON.fromJson(json, type) : new java.util.ArrayList<>();
    }
    ...
    public static synchronized void savePremiumInventory(Context context, java.util.List<InventoryItem> items) {
        ...
        boolean ok = PersistedBlobStore.save(context, PREFS_NAME, PREMIUM_KEY, INVENTORY_SCHEMA_VERSION, json);
        if (!ok) {
            android.util.Log.w("InventoryManager", "Failed to persist premium inventory");
        }
    }
    ...
    public static synchronized java.util.List<String> getInventoryChangeLog(Context context) {
        ...
        if (json != null) {
            try {
                ...
                java.util.List<String> parsed = GSON.fromJson(json, type);
                if (parsed != null) {
                    log.addAll(parsed);
                }
