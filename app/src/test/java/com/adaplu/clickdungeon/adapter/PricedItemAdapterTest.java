...
public class PricedItemAdapterTest {
    ...
    public void bindDisplaysNameAndPrice() {
        ...
        // Price label should include the localized quantity format
        android.content.Context ctx = ApplicationProvider.getApplicationContext();
        String expectedQty = ctx.getString(R.string.inventory_quantity_format, item.getQuantity());
        assertTrue(price.getText().toString().contains("Gold"));
        assertTrue(price.getText().toString().contains(expectedQty));
    }
